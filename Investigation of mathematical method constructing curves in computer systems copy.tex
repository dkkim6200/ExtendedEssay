...
%			\draw (1pt,\y) -- (-1pt,\y) node[anchor=east,fill=white] {$\ytext$};
		...
		\path	(1,1) node(p0) {} 
			(3,2) node(p1) {};
		\filldraw (p0) circle (1pt) node[align=center, below left] {$P_A$};
		\filldraw (p1) circle (1pt) node[align=center, above right] {$P_B$};
		\draw (p0) -- (p1);
		\draw[ultra thick] (p0) -- (1.5,1.25);
		\filldraw (1.5,1.25) circle (1pt) node[align=center, below right] {$P(0.25)$};
...
Mathematically, the position of a person can be expressed as the following vector equation:

\begin{equation}
P(t) = P_A + (P_B-P_A)t.
\end{equation}

Rearranging the equation as following emphasizes the comparison between $t$ and $(1-t)$.

\begin{equation}
P(t) = P_A(1-t) + P_Bt.
\end{equation}

This linear interpolation can be performed with 3 points. If we have three points $P_0 = (1,1)$, $P_1 = (2,2)$, and $P_2 = (3,1)$, we can produce straight lines between these three points using the equation of linear interpolation $P(t) = P_A(1-t) + P_{B}t$ as Fig. 2.2 shown below.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[scale=1.2]
		...
		\draw (1,1) -- (2,2) -- (3,1);
	\end{tikzpicture}
	\caption{Linear interpolants between $P_0, P_1$, and $P_1, P_2$}
\end{figure}

The interpolated point between $P_0, P_1$ is notated $P_{0...1}$, which is $(1.5, 1.5)$ at $t=0.5$. Similarly, $P_{1 \dotsc 2}$ is $(2.5, 1.5)$ at $t=0.5$. These newly interpolated points can be used as the starting and the terminal points of new linear interpolation, as shown in Fig. 2.3.
...
%	\end{scriptsize}
	\end{tikzpicture}
	\caption{Linear interpolant between $P_{0 \dotsc 1}, P_{1 \dotsc 2}$}
\end{figure}

Finally, let $P_{0 \dotsc 2}$ be the linearly interpolated point between $P_{0 \dotsc 1}, P_{1 \dotsc 2}$ at $t=0.5$. $P_{0 \dotsc 2}$, then, is a resulting point of the quadratic interpolation from points $P_0, P_1, P_2$ at $t=0.5$, as shown in Fig. 2.4.
...
If we apply the above linear interpolation repeatedly with $t=0.25$ and $t=0.75$, then we will have points shown in Fig. 2.5 and 2.6.
...
\begin{figure}[H]
	...
	\begin{minipage}{.5\textwidth}
    		...
    		\begin{footnotesize}
	    		...
	    		\filldraw (2.5,1.375) circle (1pt) node[align=center, below left] {\scriptsize $P_{0 \dotsc 2}(2.5,1.375)$};
	    	\end{footnotesize}
	    	\end{tikzpicture}
...
Firstly, the points $P_{0 \dotsc 1}, P_{1 \dotsc 2}, P_{2 \dotsc 3}$ are interpolated. These points are used to produce another set of linear interpolants, as shown in Fig. 2.9.
...
Then, the points $P_{0 \dotsc 1}, P_{1 \dotsc 2}, P_{2 \dotsc 3}$ are used to produce points $P_{0...2}, P_{1...3}$, as shown in Fig. 2.10.
...
Finally, the resulting point, $P_{0 \dotsc 3}$, of the cubic interpolation at $t=0.5$ is obtained from points $P_{0 \dotsc 2}, P_{1 \dotsc 3}$, as shown in Fig. 2.11:
...
A pattern that imitates binomial expansion of $((1-t) + t)^n$ emerges, along with the control points multiplied as coefficients of each terms as noted by \citeA{PRAUTZ:2013}. Continuing on a similar manner, the complete equation of interpolated B\'ezier curve is obtained.
...
Thus far, we have investigated on the mathematical development of B\'ezier curves from the De Casteljau algorithm. However, before the attempt of approximating a cycloid can be made, the knowledge of definition and some important properties of B\'ezier curves must precede.

To reiterate, the definition of B\'ezier curves is

\begin{equation}
b_n(t) = P_{0 \dotsc n} = \sum_{i=0}^{n} {P_i \binom{n}{i} (1-t)^{n-1} t^i} \text{\quad\cite{PRAUTZ:2013}}.
\end{equation}

Although there are numerous properties of B\'ezier curves, this investigation will utilize the following two. The first property is that, at $t=0$ and $t=1$, the interpolated points coincide with the first and last B\'ezier control points, respectively. The second property is that the tangents at $t=0$ and $t=1$ pass through points $P_0$, $P_1$ and $P_{n-1}$, $P_n$, respectively \cite{PRAUTZ:2013}. In the diagram below, the control points $P_0$ and $P_3$ explain the first property, and $P_1$ and $P_2$ lies on the tangents at $t=0$ and $t=1$ of the B\'ezier curve, respectively, which explains the second property.
...
Because this curve repeats infinitely, approximating one arc will yield a complete repeatable approximation of the curve. Moreover, since an arc of a cycloid is symmetrical at $\theta = \pi$, we only need to approximate $\theta \in [0, \pi]$ section of the arc, which is illustrated in Fig. 4.2.
...
\begin{equation_nogap}\begin{aligned}
\frac{dy}{dx}|_{t=1} 	&= \frac{\pi\sin(\pi * 0)}{\pi - \pi\cos(\pi * 0)} \\
...
This approximation is shown in Fig. 4.3.
...
We know that the initial and terminal control points ($P_0$ and $P_3$, respectively) coincides with the interpolated point of the B\'ezier curve will be at $t=0$ and $t=1$, respectively. Moreover, the second and third control points ($P_1$ and $P_2$, respectively) will lay on the tangent of the original curve at $t=0$ and $t=1$ if the approximation were to tangentially coincide with the original curve at the initial and terminal points. Graphically, this is shown in Fig. 4.8.
...
Although this appears to closely approximate the original cycloid, it is not as good as the previous approximation that utilized the B\'ezier semicircle of Dokken et al. (1990), which had $\max\limits_{t \in [0, 1]} \epsilon (t)$ of 0.0575, compared to $\approx$ 0.0810 of this approximation. However, this approximation fits the boundary conditions of tangentially coinciding with the original cycloid at $t=0, 1$.
